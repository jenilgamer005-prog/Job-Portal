  ...
  const fetchSavedItems = async () => {
    ...
    try {
      ...
      const [savedRes, rolesRes, companiesRes] = await Promise.all([
        fetch("https://job-portal-backend-gamma-two.vercel.app/api/saved", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch("https://job-portal-backend-gamma-two.vercel.app/api/interview/roles"),
        fetch("https://job-portal-backend-gamma-two.vercel.app/api/interview/companies"),
      ]);
  ...
  const toggleSave = async (rawId) => {
    try {
      ...
      const res = await fetch(
        `https://job-portal-backend-gamma-two.vercel.app/api/saved/question/${id}?type=${kind}`,
        {
          method: "POST",
          headers: {
    ...
    return savedItems.jobs.map((job) => ({
      ...
      job: {
        ...
        logo: job.companyLogo?.startsWith("http")
          ? job.companyLogo
          : `https://job-portal-backend-gamma-two.vercel.app/${job.companyLogo || ""}`,
